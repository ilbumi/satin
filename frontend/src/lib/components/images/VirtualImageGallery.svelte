<script lang="ts">
	import { Button, Card, Input, Select, Spinner, VirtualGrid } from '$lib/components/ui';
	...
	let {
		...
		onLoadMore,
		onSelectionChange
	}: VirtualImageGalleryProps = $props();
	...
	// Grid size configurations for virtual grid
	const gridConfigs = {
		sm: { itemWidth: 160, itemHeight: 140 },
		md: { itemWidth: 200, itemHeight: 180 },
		lg: { itemWidth: 280, itemHeight: 220 }
	};

	const currentGridConfig = $derived(gridConfigs[gridSize]);

	function keyExtractor(image: ImageSummary, _index: number): string {
		return image.id;
	}
</script>
...
<div class="space-y-6">
	...
	{:else}
		...
		<div>
			<VirtualGrid
				items={images}
				itemHeight={currentGridConfig.itemHeight}
				itemWidth={currentGridConfig.itemWidth}
				{containerHeight}
				{keyExtractor}
				gap={16}
				overscan={1}
			>
				{#snippet children(image: ImageSummary, _index: number)}
					<div class="p-2">
						<ImageThumbnail
							{image}
							size={gridSize}
